 ...
 * ЛОГІКА:
 * - Приймальник: видаляє записи, де pruimalnyk = його ПІБ
 * - Адміністратор: видаляє ТІЛЬКИ записи, де pruimalnyk пустий (null)
 */
async function deleteProcessedChanges(actId: number): Promise<void> {
  // ✅ Для Приймальника - видаляємо записи, де pruimalnyk = його ПІБ
  if (userAccessLevel === "Приймальник") {
    const userData = getSavedUserDataFromLocalStorage();
    const currentUserName = userData?.name || null;
  ...
  if (userAccessLevel === "Адміністратор") {
    ...
    console.log(
      `🗑️ Видалено записи з пустим pruimalnyk для акту #${actId} (Адміністратор)`
    );
    return;
  }

  // ✅ Для інших ролей - видалення недоступне
  console.log(
...
export async function checkAndHighlightChanges(actId: number): Promise<void> {
  ...
  try {
    ...
    // ✅ Видаляємо записи:
    // - Приймальник: свої записи (pruimalnyk = його ПІБ)
    // - Адміністратор: записи з пустим pruimalnyk
    await deleteProcessedChanges(actId);

    // Знімаємо синю підсвітку з акту в таблиці (для обох ролей)
    if (
      userAccessLevel === "Приймальник" ||
      userAccessLevel === "Адміністратор"
    ) {
    ...
    console.log(
      `✅ Підсвічування завершено, відповідні записи видалено (${userAccessLevel})`
    );
  } catch (error) {
